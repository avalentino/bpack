...
@pytest.mark.parametrize(
    argnames="baseunits",
    argvalues=[EBaseUnits.BYTES, EBaseUnits.BITS, "bits", "bytes"],
)
...
@pytest.mark.parametrize(
    argnames="order",
    argvalues=[EBitOrder.LSB, EBitOrder.MSB, EBitOrder.DEFAULT, "<", ">", ""],
)
...
def test_bitorder_in_byte_descriptors():
    ...
    with pytest.raises(ValueError):
        ...
        @bpack.descriptor(
            bitorder=EBitOrder.DEFAULT, baseunits=EBaseUnits.BYTES
        )
        class Record:
            field_1: int = bpack.field(size=8, default=0)
...
def test_repeat_typing():
    @bpack.descriptor
    class Record:
        field_1: typing.List[int] = bpack.field(size=4, default=0, repeat=2)
        field_2: float = bpack.field(size=8, default=1 / 3)
...
def test_no_repeat_typing():
    with pytest.raises(TypeError):
        ...
        @bpack.descriptor
        class Record:
            field_1: typing.List[int] = bpack.field(size=4, default=0)
            field_2: float = bpack.field(size=8, default=1 / 3)
...
class TestExplicitSize:
    ...
    def test_inconsistent_explicit_size_and_repeat_typing():
        with pytest.raises(bpack.descriptors.DescriptorConsistencyError):
            ...
            class Record:
                field_1: typing.List[int] = bpack.field(
                    size=4, default=0, repeat=2
                )
                field_2: float = bpack.field(size=8, default=1 / 3)
    ...
    def test_inconsistent_explicit_size_offset_and_repeat_typing():
        with pytest.raises(bpack.descriptors.DescriptorConsistencyError):
            ...
            class Record:
                field_1: int = bpack.field(size=4, default=0)
                field_2: typing.List[float] = bpack.field(
...
def test_sequence_type():
    ...
    @bpack.descriptor
    class Record:
        field_1: typing.List[int] = bpack.field(size=1, repeat=4)

    field_1 = bpack.fields(Record)[0]
    assert field_1.type is typing.List[int]
    assert bpack.calcsize(Record) == 4
...
@pytest.mark.parametrize("baseunits", [EBaseUnits.BYTES, EBaseUnits.BITS])
